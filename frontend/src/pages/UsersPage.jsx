...
const ACCESS = { owner: "Seluruh sistem", admin: "Operasional umum: siswa, kelas, job order, laporan", finance: "Pembayaran, pemasukan, pengeluaran, kas, laporan keuangan", hr: "Karyawan, guru, laporan SDM", guru: "Kelas yang diajar, absensi, nilai", marketing: "Calon siswa, pendaftaran, job order", staff: "Akses terbatas (baca siswa & kelas)" };
const EMPTY = { name: "", email: "", password: "", role: "staff", employee_id: "", aktif: true };
...
export default function UsersPage() {
  ...
  const save = async () => {
    const body = { ...form, employee_id: form.employee_id || null, password: form.password || null };
  ...
  return (
    <div>
      <PageHeader title="Pengguna & Hak Akses" jp="ユーザー管理" subtitle="Akun dibuat oleh Super Admin. Setiap role hanya melihat modul yang relevan.">
        <button className="btn-red" onClick={() => { setForm(EMPTY); setEditId(null); setOpen(true); }} data-testid="add-user-btn"><Plus size={16} />Tambah Akun</button>
      </PageHeader>
      ...
      {loading && !data ? <Loading /> : (
        <div className="table-wrap fade-up"><table className="tbl" data-testid="users-table"><thead><tr><th>Nama</th><th>Email</th><th>Role</th><th>Terhubung ke</th><th>Status</th><th>Login Terakhir</th><th></th></tr></thead>
          <tbody>{(data || []).map((u) => <tr key={u.id} data-testid={`user-row-${u.id}`}><td className="font-semibold">{u.name}</td><td>{u.email}</td><td><span className="chip bg-slate-900 text-white border-slate-900">{ROLE_LABELS[u.role]}</span></td><td className="text-xs">{(emps || []).find((e) => e.id === u.employee_id)?.nama || "-"}</td><td><span className={`chip ${u.aktif !== false ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-slate-100"}`}>{u.aktif !== false ? "Aktif" : "Nonaktif"}</span></td><td className="text-xs">{fmtDateTime(u.last_login)}</td>
            <td className="whitespace-nowrap"><button className="btn-ghost btn-sm" onClick={() => { setForm({ ...EMPTY, ...u, password: "", employee_id: u.employee_id || "" }); setEditId(u.id); setOpen(true); }} data-testid={`edit-user-${u.id}`}><Pencil size={13} /></button>{u.id !== me.id && <button className="btn-ghost btn-sm text-red-600" onClick={() => remove(u)} data-testid={`delete-user-${u.id}`}><Trash2 size={13} /></button>}</td></tr>)}
          </tbody></table></div>)}
      <FormDialog open={open} onOpenChange={setOpen} title={editId ? "Edit Akun" : "Tambah Akun Pengguna"} onSubmit={save} testId="user-dialog">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Role"><select className="input" data-testid="user-role-select" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>{Object.entries(ROLE_LABELS).map(([k, l]) => <option key={k} value={k}>{l}</option>)}</select></Field>
